...
use crate::{Unit, data::InstData, data::Instruction, data::Block};
...
impl Unit {
    ...
    pub(crate) fn block_adjacency(&self) -> (Vec<u32>, QuadVec<bool>) {
		...
		while let Some(block) = queue.get(counter).cloned() {
    		...
    		pass.map(|p| { col[p.0 as usize] = true; queue.push(p.0) });
    		next.map(|p| { col[p.0 as usize] = true; queue.push(p.0) });
    		counter += 1;
		}
		(queue, out)
    }
